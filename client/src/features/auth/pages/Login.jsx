...
const Login = () => {
    ...
    if (loading) {
        return <LoadingSkeleton label="Loading sign in" />
    }
                ...
                <Card className="relative mx-auto w-full max-w-md border-white/15 bg-white/10 text-white shadow-floating backdrop-blur">
                    <CardContent className="space-y-5">
                        ...
                        <div className="rounded-2xl bg-white/10 p-5">
                            <p className="text-sm text-white/70">Role Match</p>
                            <strong className="text-6xl font-black tracking-[-0.06em]">86%</strong>
                        </div>
                        <div className="grid gap-3">
                            <span className="h-3 rounded-full bg-white/20" />
                            <span className="h-3 w-3/4 rounded-full bg-white/20" />
                            <span className="h-3 w-1/2 rounded-full bg-white/20" />
                        </div>
                    </CardContent>
                </Card>
                <div className="relative max-w-xl">
                    <h2 className="text-5xl font-black leading-none tracking-[-0.06em]">Walk into every interview with a sharper plan.</h2>
                    <p className="mt-4 leading-7 text-white/70">Generate targeted questions, identify skill gaps, and build a focused prep roadmap from your resume and job description.</p>
                </div>
            </section>
                ...
                <Card className="w-full max-w-lg animate-fade-up">
                    <CardContent className="space-y-6 p-6 sm:p-10">
                        <div className="space-y-2">
                            <p className="text-xs font-black uppercase tracking-[0.08em] text-brand-600">Welcome back</p>
                            <h1 className="text-4xl font-black tracking-[-0.05em] text-slate-950">Sign in to your workspace</h1>
                            <p className="text-sm leading-6 text-slate-600">Continue building interview strategies tailored to your next role.</p>
                        </div>
                        ...
                        <p className="text-center text-sm text-slate-600">Don't have an account? <Link className="font-black text-brand-600 hover:text-brand-700" to="/register">Create one</Link></p>
                    </CardContent>
                </Card>
            </section>
        </main>
